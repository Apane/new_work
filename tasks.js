...
have not unlocked the feature, then we should not tell them how many people
...
-In settings, people should be able to hide events and activities
the have been to so that it does not show up on their profile.
...
??? - Ability to point over someone and find out more information about them.
...
- When I am in Browse, can we make the age From and To a text box instead of a drop down menu.
  And if people put anyone puts an invalid number then default to 18.  Look at OKCupid for this.
...
// no such attribute as employer, was not in new design either
// Employer:
...
!!! -The Top 5s are not working the way that was in the designs.
("he's talking about the new design for Top 5 with all the little boxes.")
...
*** -Description box should be rich text box as in the Designs.
The description box UI also needs to be corrected to look more like the mocks.

- For Event Image, we need to make sure the resolution is straight for all of them, the same that goes for profile picture.
  This way the image do not look distorted.

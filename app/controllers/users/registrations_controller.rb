class Users::RegistrationsController < Devise::RegistrationsController
  ...
  def update
    if resource.update_attributes(params[resource_name])
      ...
      sign_in resource_name, resource, :bypass => true # Line required if using Devise >= 1.2.0
      redirect_to :back #after_update_path_for(resource)
    else
      clean_up_passwords(resource)
      redirect_to :back, notice: 'Account was not updated!'
    end
  end
